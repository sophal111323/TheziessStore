...
export default function AdminSidebar({
  ...
  return (
    <aside className="admin-sidebar group relative flex h-screen w-64 shrink-0 flex-col overflow-hidden border-r border-pink-200/80 bg-pink-50/80 text-pink-900 shadow-[18px_0_48px_rgba(147,51,234,0.10)] backdrop-blur-xl">
      ...
      <nav className="relative z-10 flex-1 overflow-y-auto px-3 py-4">
        <div className="relative space-y-1.5">
          <span
            aria-hidden="true"
            className="admin-sidebar-indicator absolute left-0 right-0 top-0 h-11 rounded-2xl border border-pink-300/90 bg-white/85 shadow-[0_10px_30px_rgba(147,51,234,0.16)] ring-1 ring-white/70"
          ...
          {ADMIN_NAV_ITEMS.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <span className="ml-auto h-2 w-2 rounded-full bg-pink-500 shadow-[0_0_14px_rgba(147,51,234,0.65)]" />
                )}
              </Link>
            );
          })}
