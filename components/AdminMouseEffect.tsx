  ...
  useEffect(() => {
    ...
    const drawTrail = (now: number, fade: number) => {
      ...
      for (let i = 1; i < liveTrail.length; i += 1) {
        ...
        ctx.strokeStyle = `rgba(147, 51, 234, ${alpha * 0.42})`;
        ctx.lineWidth = 1.4 + alpha * 3.2;
        ctx.shadowColor = `rgba(168, 85, 247, ${alpha * 0.42})`;
    ...
    const drawRipples = (now: number, fade: number) => {
      for (let i = ripples.length - 1; i >= 0; i -= 1) {
        ...
        ctx.save();
        ctx.strokeStyle = `rgba(168, 85, 247, ${alpha * 0.44})`;
        ctx.lineWidth = 2.2 - progress;
        ctx.shadowColor = `rgba(147, 51, 234, ${alpha * 0.5})`;
    ...
    const drawCursor = (now: number, fade: number) => {
      ...
      ctx.strokeStyle = "rgba(168, 85, 247, 0.64)";
      ctx.lineWidth = 1.55;
      ...
      glow.addColorStop(0.45, "rgba(192, 132, 252, 0.34)");
      ...
      fill.addColorStop(0.45, "#c084fc");
      fill.addColorStop(1, "#7e22ce");
      ctx.fillStyle = fill;
      ctx.shadowColor = "rgba(126, 34, 206, 0.6)";
      ctx.shadowBlur = 12;
      ctx.fill();
