...
export default function AdminOrderDetailPage() {
  ...
  async function callAutoFulfill() {
    ...
    try {
      const res = await fetch(`/api/admin/orders/${orderNumber}/fulfill`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          force: order?.status === "FAILED" || order?.status === "DELIVERED" || order?.status === "PENDING",
        }),
      });
      const data = await res.json().catch(() => null);
      if (res.ok && data?.success) {
        alert(`✅ Top-up API called successfully!\nRef: ${data.transactionId || data.status || "Completed"}`);
      } else {
        alert(`❌ Top-up API call failed: ${data?.error || data?.status || "Unknown error"}`);
      }
      await load();
    } catch (err: any) {
      alert(`❌ Error calling top-up API: ${err?.message || "Network error"}`);
    } finally {
      setUpdating(false);
    }
  }
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex justify-between items-start mb-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={callAutoFulfill}
            disabled={updating}
            className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2 text-xs font-bold text-white shadow-lg shadow-orange-500/20 hover:from-amber-600 hover:to-orange-600 hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-50"
          >
            <span>⚡</span>
            <span>{updating ? "Calling…" : order.status === "DELIVERED" ? "Re-call API" : "Call API Again"}</span>
          </button>
          ...
          <div className={`mb-6 rounded-2xl border p-6 transition-colors ${
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                onClick={callAutoFulfill}
                disabled={updating}
                className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-orange-500/30 transition-all hover:shadow-orange-500/50 hover:-translate-y-0.5 disabled:opacity-50"
              >
                <span>⚡</span>
                <span>{updating ? "Calling API…" : "Call Top-up API Again"}</span>
              </button>
