...
export async function fulfillPaidOrder(
  ...
  if (order.status === "DELIVERED") {
    ...
    // Admin force-retry on delivered order: reset to PAID with failed topupStatus to allow new API call
    await prisma.order.update({
      where: { id: order.id },
      data: { status: "PAID", topupStatus: "failed" },
    });
    order.status = "PAID";
  }

  // Allow retrying failed orders
  ...
  // If force is requested on PENDING order, allow admin override
